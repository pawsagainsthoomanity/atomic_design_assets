...
const checkIsDoczRepo = assign(ctx => {
  const isDoczRepo = sh.test('-e', join(root, '../../core'));
...
const getPattern = config => {
  ...
  return ignore.map(entry => typeof entry === 'string' && `!**/${entry}`).filter(Boolean).concat([join(src, ts ? '**/*.{ts,tsx}' : '**/*.{js,jsx,mjs}'), '!**/node_modules', '!**/doczrc.js']);
};
